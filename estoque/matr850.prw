...
//³                                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("MATR850",STR0001,"MTR850", {|oReport| ReportPrint(oReport,@cAliasSC2,@cAliasSD3)},STR0002) //"Este programa ira imprimir a Rela‡„o das Ordens de Produ‡„o."
If nTamProd > 15
...
//  mv_par12			// tipo de - ja existente
//  mv_par13            // tipo até - ja existente
//  mv_par14            // Data de entrega?   - novo           
//  mv_par15            // Ate a data de entrega?   - novo        
//  mv_par16            // Data real fim?   - novo                
//  mv_par17            // Ate Data real fim?   - novo            
//  mv_par18            // Pedido de?   - novo                    
//  mv_par19            // Pedido até?   - novo                   
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
TRCell():New(oOp,'C2_QUJE'		,'SC2',"QTD.APONT",cPictQtd	,nTamSld,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oOp,'ZL2_QUANT'	,'SC2',"ARMAZEM 01",cPictQtd	,nTamSld,/*lPixel*/,/*{|| code-block de impressao }*/)   
...
TRCell():New(oOp,'C2_PEDIDO'	,'SC2',"Pedido",/*Picture*/	,/*lPixel*/,/*{|| code-block de impressao }*/) //Marcio Nunes
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function ReportPrint(oReport,cAliasSC2, cAliasSD3)
...
BeginSql Alias cAliasSC2
	...
	C2_EMISSAO,C2_DATPRF,C2_QUANT,C2_QUJE,C2_PERDA,C2_STATUS,C2_TPOP, C2_PEDIDO, 
	ISNULL((SELECT SUM(ZL2_QUANT) AS ZL2_QUANT FROM %table:ZL2% WHERE ZL2_OP=SC2.C2_NUM+SC2.C2_ITEM+SC2.C2_SEQUEN 
	AND ZL2_COD=SC2.C2_PRODUTO AND ZL2_DTIMPR<>'' AND ZL2_ENDER='S' AND SC2.C2_QUJE > 0 GROUP BY ZL2_OP),0) AS ZL2_QUANT,
	...
	//Marcio Nunes - 13/11/2019 - Cha,ado: 71B-17V-DX7Z
	//  mv_par14            // Data de entrega?   - novo           
	//  mv_par15            // Ate a data de entrega?   - novo
	SC2.C2_DATPRF  >= %Exp:mv_par14% AND SC2.C2_DATPRF  <= %Exp:mv_par15% AND        
	//  mv_par16            // Data real fim?   - novo                
	//  mv_par17            // Ate Data real fim?   - novo                                                 
	SC2.C2_DATRF  >= %Exp:mv_par16% AND SC2.C2_DATRF  <= %Exp:mv_par17% AND     
	//  mv_par18            // Pedido de?   - novo                    
	//  mv_par19            // Pedido até?   - novo                                                        
	SC2.C2_PEDIDO  >= %Exp:mv_par18% AND SC2.C2_PEDIDO  <= %Exp:mv_par19% AND
	
	
	SB1.B1_TIPO  BETWEEN %Exp:mv_par12% AND %Exp:mv_par13% AND
...
If !tamsx3('B1_COD')[1] > 15
	cabec1 := If(!__lPyme, IIf(lTipo,STR0016,STR0011), IIf(lTipo,STR0018,STR0015))//"NUMERO         P R O D U T O                                                  CENTRO    EMISSAO      ENTREGA     ENTREGA       QUANTIDADE          SALDO A      ST TP		PEDIDO"
	cabec2 := IIf(lTipo,STR0017,STR0012)	//"                                                                            DE CUSTO                PREVISTA        REAL         ORIGINAL         ENTREGAR"
Else
	cabec1 := if(!__lPyme, IIF(lTipo,SUBSTR(STR0016,1,78)+SPACE(nColadd)+SUBSTR(STR0016,79,166),SUBSTR(STR0011,1,78)+SPACE(nColadd)+SUBSTR(STR0011,79,166)), IIf(lTipo,SUBSTR(STR0018,1,78)+SPACE(nColadd)+SUBSTR(STR0018,79,166),SUBSTR(STR0015,1,78)+SPACE(nColadd)+SUBSTR(STR0015,79,166)+SPACE(nColadd)+SPACE(nColadd)+"PED. VENDAS"))
...
cAliasSC2 := "R850IMP"
	...
	cQuery    += "SC2.C2_STATUS,SC2.C2_TPOP, SC2.C2_PEDIDO, "
	...
	//  mv_par14            // Data de entrega?   - novo           
	//  mv_par15            // Ate a data de entrega?   - novo
	cQuery    += "SC2.C2_DATPRF  >= '"+DtoS(mv_par14)+"' And SC2.C2_DATPRF  <= '"+DtoS(mv_par15)+"' And "        
	//  mv_par16            // Data real fim?   - novo                
	//  mv_par17            // Ate Data real fim?   - novo                                                 
	cQuery    += "SC2.C2_DATRF  >= '"+DtoS(mv_par16)+"' And SC2.C2_DATRF  <= '"+DtoS(mv_par17)+"' And "
	//  mv_par18            // Pedido de?   - novo                    
	//  mv_par19            // Pedido até?   - novo                                                        
	cQuery    += "SC2.C2_PEDIDO  >= '"+mv_par18+"' And SC2.C2_PEDIDO  <= '"+mv_par19+"' And "    

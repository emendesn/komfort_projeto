...
//Tratamento para checar o saldo por endere็o dos produtos e nใo deixar faturar quando nใo existir saldo
TRBPED->(DbGoTop())
While TRBPED->(!Eof())
	If !Empty(TRBPED->PED_MARCA) //Itens marcados
		If SC6->(DbSeek(xFilial("SC6")+TRBPED->PED_PEDIDO + TRBPED->PED_ITEM ))
			cLocaliz := SC6->C6_LOCALIZ
			cCodigo:=SC6->C6_PRODUTO
			cArmazem:=SC6->C6_LOCAL
			nSldC6:=SC6->C6_QTDVEN
			
			_cQuery := " SELECT B2_COD, B2_QATU, B2_LOCAL FROM "+ RETSQLNAME("SB2") + " SB2 (NOLOCK) "
			_cQuery += " WHERE  SB2.D_E_L_E_T_=' ' AND SB2.B2_LOCAL='"+cArmazem+"' AND SB2.B2_COD='"+cCodigo+"'"
			
			_cQuery := ChangeQuery(_cQuery)
			
			DbUseArea( .T., "TOPCONN", TCGenQry( Nil, Nil, _cQuery ), _cAlias, .T., .F. )
			
			Sldb2:=(_cAlias)->B2_QATU
			
			(_cAlias)->(dbCloseArea())
			
			If Sldb2-nSldC6 < 0
				MsgAlert("Nใo foi possivel realizar a montagem de carga por falta de saldo em estoque do Pedido "+ TRBPED->PED_PEDIDO + " Item: "+TRBPED->PED_ITEM+ " Acione o SAC " ,"ATENวรO")
				lContinua	:= .F.
			Endif
			
			If Localiza(cCodigo)
				If Empty(cLocaliz)
					MsgAlert("O Endere็o do produto estแ em branco, por favor verfiique o pedido e o item a seguir "+ TRBPED->PED_PEDIDO + " Item: "+TRBPED->PED_ITEM+ " Acione o SAC " ,"ATENวรO")
					lContinua	:= .F.
				Endif
			Endif
		Endif

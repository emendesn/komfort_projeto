...
±±³Programa  ³KMFINR05³ Autor ³ Microsiga             ³ Data ³ 13/10/03 ³±±
...
/*/
//User Function KMFINR05( lJob, _nRecno )		//Alteracao par envio por JOB --> 001
User Function KMFINR05(_cPed,_aTitulos)		//Alteracao par envio por JOB --> 001

Local _cBolItau	 := AllTrim(GetMV("KH_BOLITAU",,""))
Local _nx := 0
Local _nCont := 1
Private _cBanco    := ""
Private _cAG       := ""
...
Else
	...
	For _nx := 1 To Len(_cBolItau)
		
		If _nCont == 1 .And. Subs(_cBolItau,_nx,1) <> '/'
			
			_cBanco += Subs(_cBolItau,_nx,1)
			
		ElseIf _nCont == 2 .And. Subs(_cBolItau,_nx,1) <> '/'
			
			_cAg += Subs(_cBolItau,_nx,1)
			
		ElseIf _nCont == 3 .And. Subs(_cBolItau,_nx,1) <> '/'
			
			_cCC += Subs(_cBolItau,_nx,1)
...
Else
	...
	If !(Alltrim(cUserId) $ _cGrpCred) //Não for usuário credito
	
		If MsgYesNo("Deseja imprimir por cliente?","IMPRICLI")
			_lImpCli := .T.
		EndIf
...
If !_lAuto
	...
	wnrel    := "BOLETO_ITAU"
	lEnd     := .F.
	_cPerg     := PADR("BLTITAU",LEN(SX1->X1_GRUPO))
	_cPerg2    := PADR("BLTITAU2",LEN(SX1->X1_GRUPO))
	...
	If _lImpCli
		AjustaSx1(_cPerg2,2)
		Pergunte (_cPerg2,.T.)	
		Wnrel := SetPrint(cString,Wnrel,_cPerg2,@Titulo,cDesc1,cDesc2,cDesc3,.F.,,,Tamanho,,)
	...
	If !(Alltrim(cUserId) $ _cGrpCred) //Se não for usuário do crédito
		
		If !_lImpCli
		
			cIndexName	:= Criatrab(Nil,.F.)
			cIndexKey	:= "E1_PORTADO+E1_CLIENTE+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO+DTOS(E1_EMISSAO)"
			cFilter		+= "E1_FILIAL=='"+xFilial("SE1")+"' .And. E1_SALDO>0 .And. E1_TIPO = 'BOL' .And. "
		...
		Else
			...
			cFilter		+= "E1_FILIAL=='"+xFilial("SE1")+"' .And. E1_SALDO>0 .And. E1_TIPO = 'BOL' .And. "
			cFilter		+= "E1_PREFIXO>='' .And. E1_PREFIXO<='ZZZ' .And. "
			cFilter		+= "E1_NUM>='' .And. E1_NUM<='ZZZZZZZZZ' .And. "
			cFilter		+= "E1_PARCELA>='' .And. E1_PARCELA<='Z' .And. "
			cFilter		+= "E1_CLIENTE = '" + MV_PAR01 + "' .And.  "
			cFilter		+= "E1_LOJA>='" + MV_PAR02 + "' .And.  "
			cFilter		+= "DTOS(E1_EMISSAO)>='"+DTOS(mv_par03)+"' .and. DTOS(E1_EMISSAO)<='"+DTOS(mv_par04)+"' .And."
			cFilter		+= "E1_NUMBCO <> ' ' .And. "
	...
	Else
		...
		cFilter		+= "E1_FILIAL=='"+xFilial("SE1")+"' .And. E1_SALDO>0 .And. E1_TIPO = 'BOL' .And. "
		cFilter		+= "E1_PREFIXO>='' .And. E1_PREFIXO<='ZZZ' .And. "
		cFilter		+= "E1_NUM>='' .And. E1_NUM<='ZZZZZZZZ' .And. "
		cFilter		+= "E1_PARCELA>='' .And. E1_PARCELA<='Z' .And. "
		cFilter		+= "E1_CLIENTE>='' .And. E1_CLIENTE<='ZZZZZZ' .And. "
		cFilter		+= "E1_LOJA>='' .And. E1_LOJA<='ZZ' .And. "
		cFilter		+= "DTOS(E1_EMISSAO)>='20120101' .and. DTOS(E1_EMISSAO)<='20301231' .And."
		cFilter		+= "DTOS(E1_VENCREA)>='20120101' .and. DTOS(E1_VENCREA)<='20301231' .And. "
		cFilter		+= "E1_NUMBOR>=' ' .And. E1_NUMBOR<= 'ZZZZZ'.And. "
		cFilter		+= "E1_NUMBCO = ' ' .And. "
...
Else
	...
	cIndexKey	:= "E1_PORTADO+E1_CLIENTE+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO+DTOS(E1_EMISSAO)"
	cFilter		+= "E1_FILIAL=='"+xFilial("SE1")+"'.And.E1_SALDO>0.And. E1_TIPO = 'BOL' .And. "
	cFilter		+= "E1_PEDIDO = '" + _cPed + "'.And. "
	cFilter		+= "E1_PORTADO = '341'.And. "
	cFilter		+= "E1_NUMBCO <> ' '.And. "
	cFilter		+= "!(E1_TIPO$MVABATIM)"
	...
	//Se envio for atraves de rotina de JOB (para email em html/pdf) não gera
	...
	cIndexName	:= Criatrab(Nil,.F.)
	cIndexKey	:= "E1_PORTADO+E1_CLIENTE+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO+DTOS(E1_EMISSAO)"
	//	cFilter		:= "RECNO()    = " + alltrim(STR( _nRecno ))
	...
	MontaRel( lJob, _nRecno )
	//Alteracao par envio por JOB --> 004/*/
Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function MontaRel()	//Alteracao par envio por JOB --> 005
LOCAL oPrint
LOCAL nX := 0
Local cNroDoc :=  " "
...
Do While !EOF()
	...
	//Substituido a pedido do Saran, para que considere o numero bancario da sequencia do SEE
	_cNossoNum := NossoNumSEE()
	
	aCB_RN_NN    := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],_cNossoNum,(E1_VALOR-nVlrAbat),E1_VENCTO)
...
oFont21  := TFont():New("Arial",9,20,.T.,.T.,5,.T.,5,.T.,.F.)
...
If lMv_Logo
	cLogo := GetSrvProfString("Startpath","") + "logo_itau" + ".BMP"
	If !File(cLogo)
		cLogo	:= GetSrvProfString("Startpath","") + "logo_itau" + ".BMP"
		If !File(cLogo)
			lMv_Logod := .F.
...
oPrint:Line (nRow2+710,98 ,nRow2+1640,98 )
oPrint:Line (nRow2+710,2300 ,nRow2+1640,2300 )

//oPrint:Say  (nRow2+0644,100,aDadosBanco[2],oFont14 )		// [2]Nome do Banco
If lMv_Logo
	oPrint:SayBitmap(nRow2+605,100,cLogo,380,100)
Else
	oPrint:Say  (nRow2+0644,100,aDadosBanco[2],oFont14 )		// [2]Nome do Banco
EndIF

oPrint:Say  (nRow2+0635,520,aDadosBanco[1]+"-7",oFont21 )	// [1]Numero do Banco
...
oPrint:Line (nRow2+1080,100,nRow2+1080,1800 )
oPrint:Line (nRow2+1010,100,nRow2+1010,1800 )
oPrint:Line (nRow2+1150,100,nRow2+1150,1800 )

oPrint:Line (nRow2+1010,500,nRow2+1150,500)
oPrint:Line (nRow2+1080,750,nRow2+1150,750)
oPrint:Line (nRow2+1010,1000,nRow2+1150,1000)
oPrint:Line (nRow2+1010,1300,nRow2+1080,1300)
oPrint:Line (nRow2+1010,1480,nRow2+1150,1480)

oPrint:Line (nRow2+0980,1800,nRow2+0980,2300 )
oPrint:Line (nRow2+1050,1800,nRow2+1050,2300 )

oPrint:Say  (nRow2+0710,100 ,"Local de Pagamento",oFont8)
oPrint:Say  (nRow2+0725,400 ,"PAGAVÉL EM QUANQUER BANCO ATÉ O VENCIMENTO ",oFont10)
...
oPrint:Say  (nRow2+0910,100 ,"Endereço Beneficiário / Sacador Avalista"                                        ,oFont8)
oPrint:Say  (nRow2+0950,100 ,AllTrim(aDadosEmp[2])+" - "+AllTrim(aDadosEmp[3])+" - "+AllTrim(aDadosEmp[4])	,oFont10) //Nome + CNPJ
...
oPrint:Say  (nRow2+1010,100 ,"Data do Documento"                              ,oFont8)
oPrint:Say  (nRow2+1040,100, StrZero(Day(aDadosTit[2]),2) +"/"+ StrZero(Month(aDadosTit[2]),2) +"/"+ Right(Str(Year(aDadosTit[2])),4),oFont10)

oPrint:Say  (nRow2+1010,505 ,"Nro.Documento"                                  ,oFont8)
oPrint:Say  (nRow2+1040,605 ,aDadosTit[7]+aDadosTit[1]						,oFont10) //Prefixo +Numero+Parcela

oPrint:Say  (nRow2+1010,1005,"Espécie Doc."                                   ,oFont8)
oPrint:Say  (nRow2+1040,1050,aDadosTit[8]										,oFont10) //Tipo do Titulo

oPrint:Say  (nRow2+1010,1305,"Aceite"                                         ,oFont8)
oPrint:Say  (nRow2+1040,1400,"N"                                             ,oFont10)

oPrint:Say  (nRow2+1010,1485,"Data do Processamento"                          ,oFont8)
oPrint:Say  (nRow2+1040,1550,StrZero(Day(aDadosTit[3]),2) +"/"+ StrZero(Month(aDadosTit[3]),2) +"/"+ Right(Str(Year(aDadosTit[3])),4),oFont10) // Data impressao
...
oPrint:Say  (nRow2+1080,100 ,"Uso do Banco"                                   ,oFont8)

oPrint:Say  (nRow2+1080,505 ,"Carteira"                                       ,oFont8)
oPrint:Say  (nRow2+1110,555 ,aDadosBanco[6]                                  	,oFont10)

oPrint:Say  (nRow2+1080,755 ,"Espécie"                                        ,oFont8)
oPrint:Say  (nRow2+1110,805 ,"R$"                                             ,oFont10)

oPrint:Say  (nRow2+1080,1005,"Quantidade"                                     ,oFont8)
oPrint:Say  (nRow2+1080,1485,"Valor"                                          ,oFont8)
...
oPrint:Say  (nRow2+1150,105 ,"APÓS O VENCIMENTO COBRAR MULTA DE R$ 35,00",oFont10)
oPrint:Say  (nRow2+1190,105 ,"NÃO RECEBER APÓS 30 DIAS DO VENCIMENTO",oFont10)
oPrint:Say  (nRow2+1230,105 ,"NÃO AUTORIZADO O RECEBIMENTO PARCIAL",oFont10)
oPrint:Say  (nRow2+1270,105 ,"SUJEITO A NEGATIVAÇÃO APÓS 30 DIAS DE ATRASO E NÃO PAGO",oFont10)
oPrint:Say  (nRow2+1310,105 ,"DISPONÍVEL PARA PAGAMENTO NA REDE BANCÁRIA A PARTIR DE "+_cDtBol,oFont10)
...
For nI := 100 to 2300 step 50
	oPrint:Line(nRow3+1800, nI, nRow3+1800, nI+30)
Next nI
...
If lMv_Logo
	oPrint:SayBitmap(nRow3+1897,100,cLogo,380,100)
Else
	oPrint:Say  (nRow3+1934,100,aDadosBanco[2],oFont14 )		// 	[2]Nome do Banco
EndIF

oPrint:Say  (nRow3+1925,520,aDadosBanco[1]+"-7",oFont21 )	// 	[1]Numero do Banco
oPrint:Say  (nRow3+1934,755,aCB_RN_NN[2],oFont15n)			//		Linha Digitavel do Codigo de Barras

oPrint:Line (nRow3+2000,98 ,nRow3+2850,98 )
...
oPrint:Say  (nRow3+2340,105 ,"APÓS O VENCIMENTO COBRAR MULTA DE R$ 35,00",oFont10)
oPrint:Say  (nRow3+2380,105 ,"NÃO RECEBER APÓS 30 DIAS DO VENCIMENTO",oFont10)
oPrint:Say  (nRow3+2420,105 ,"NÃO AUTORIZADO O RECEBIMENTO PARCIAL",oFont10)
oPrint:Say  (nRow3+2460,105 ,"SUJEITO A NEGATIVAÇÃO APÓS 30 DIAS DE ATRASO E NÃO PAGO",oFont10)
oPrint:Say  (nRow2+2500,105 ,"DISPONÍVEL PARA PAGAMENTO NA REDE BANCÁRIA A PARTIR DE "+_cDtBol,oFont10)
oPrint:Say  (nRow2+2540,105 ,"Duvidas! Cobrança Whats 97103-4439")
...
MSBAR3("INT25",21,1,aCB_RN_NN[1],oPrint,.F.,Nil,Nil,0.025,1.5,Nil,Nil,"A",.F.)
...
/*/
Static Function AjustaSX1(_cPerg,_nOpc)
...
Aadd(_aPergs,{"De Prefixo","","","mv_ch1","C",3,0,0,"G","","MV_PAR01","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Prefixo","","","mv_ch2","C",3,0,0,"G","","MV_PAR02","","","","ZZZ","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"De Numero","","","mv_ch3","C",6,0,0,"G","","MV_PAR03","","","","","","","","","","","","","","","","","","","","","","","","","","SE1","","",""})
Aadd(_aPergs,{"Ate Numero","","","mv_ch4","C",6,0,0,"G","","MV_PAR04","","","","ZZZZZZ","","","","","","","","","","","","","","","","","","","","SE1","","","","",""})
Aadd(_aPergs,{"De Parcela","","","mv_ch5","C",1,0,0,"G","","MV_PAR05","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Parcela","","","mv_ch6","C",1,0,0,"G","","MV_PAR06","","","","Z","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"De Cliente","","","mv_ch9","C",6,0,0,"G","","MV_PAR07","","","","","","","","","","","","","","","","","","","","","","","","","SA1","","","",""})
Aadd(_aPergs,{"Ate Cliente","","","mv_cha","C",6,0,0,"G","","MV_PAR08","","","","ZZZZZZ","","","","","","","","","","","","","","","","","","","","","SA1","","","",""})
Aadd(_aPergs,{"De Loja","","","mv_chb","C",2,0,0,"G","","MV_PAR09","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Loja","","","mv_chc","C",2,0,0,"G","","MV_PAR10","","","","ZZ","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"De Emissao","","","mv_chd","D",8,0,0,"G","","MV_PAR11","","","","01/01/80","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Emissao","","","mv_che","D",8,0,0,"G","","MV_PAR12","","","","31/12/03","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"De Vencimento","","","mv_chf","D",8,0,0,"G","","MV_PAR13","","","","01/01/80","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Vencimento","","","mv_chg","D",8,0,0,"G","","MV_PAR14","","","","31/12/03","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Do Bordero","","","mv_chh","C",6,0,0,"G","","MV_PAR15","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Bordero","","","mv_chi","C",6,0,0,"G","","MV_PAR16","","","","ZZZZZZ","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Re-Impressao","","","mv_chj","N",1,0,0,"C","","MV_PAR17","Sim","Si","Yes","","","Não","No","Not","","","","","","","","","","","","","","","","","","","","",""})
...
dbSelectArea("SX1")
dbSetOrder(1)
For nX:=1 to Len(_aPergs)
	lAltera := .F.
	If MsSeek(_cPerg+Right(_aPergs[nX][11], 2))
		If (ValType(_aPergs[nX][Len(_aPergs[nx])]) = "B" .And.;
			Eval(_aPergs[nX][Len(_aPergs[nx])], _aPergs[nX] ))
			_aPergs[nX] := ASize(_aPergs[nX], Len(_aPergs[nX]) - 1)
			lAltera := .T.
		Endif
	Endif
	
	If ! lAltera .And. Found() .And. X1_TIPO <> _aPergs[nX][5]
	...
	If ! Found() .Or. lAltera
		...
		Replace X1_ORDEM with Right(_aPergs[nX][11], 2)
		For nj:=1 to Len(aCposSX1)
			If 	Len(_aPergs[nX]) >= nJ .And. _aPergs[nX][nJ] <> Nil .And.;
				FieldPos(AllTrim(aCposSX1[nJ])) > 0
				Replace &(AllTrim(aCposSX1[nJ])) With _aPergs[nx][nj]
			Endif
		Next nj
		MsUnlock()
		cKey := "P."+AllTrim(X1_GRUPO)+AllTrim(X1_ORDEM)+"."
		
		If ValType(_aPergs[nx][Len(_aPergs[nx])]) = "A"
			aHelpSpa := _aPergs[nx][Len(_aPergs[nx])]
		Else
			aHelpSpa := {}
		Endif
		
		If ValType(_aPergs[nx][Len(_aPergs[nx])-1]) = "A"
			aHelpEng := _aPergs[nx][Len(_aPergs[nx])-1]
		Else
			aHelpEng := {}
		Endif
		
		If ValType(_aPergs[nx][Len(_aPergs[nx])-2]) = "A"
			aHelpPor := _aPergs[nx][Len(_aPergs[nx])-2]
		Else
			aHelpPor := {}
		Endif
...
Aadd(_aPergs,{"Cliente","","","mv_ch1","C",6,0,0,"G","","MV_PAR01","","","","","","","","","","","","","","","","","","","","","","","","","SA1","","","",""})
Aadd(_aPergs,{"Loja Cliente","","","mv_ch2","C",2,0,0,"G","","MV_PAR02","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"De Emissao","","","mv_ch3","D",8,0,0,"G","","MV_PAR03","","","","01/01/1980","","","","","","","","","","","","","","","","","","","","","","","","",""})
Aadd(_aPergs,{"Ate Emissao","","","mv_ch4","D",8,0,0,"G","","MV_PAR04","","","","31/12/2030","","","","","","","","","","","","","","","","","","","","","","","","",""})
...
For nX:=1 to Len(_aPergs)
	lAltera := .F.
	If MsSeek(_cPerg2+Right(_aPergs[nX][11], 2))
		If (ValType(_aPergs[nX][Len(_aPergs[nx])]) = "B" .And.;
			Eval(_aPergs[nX][Len(_aPergs[nx])], _aPergs[nX] ))
			_aPergs[nX] := ASize(_aPergs[nX], Len(_aPergs[nX]) - 1)
			lAltera := .T.
...
±±ºPrograma  ³NossoNumSEºAutor  ³Microsiga           º Data ³  05/04/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Retorna e atualiza a sequencia do nossoNumero do cadastro   º±±
±±º          ³dos parametros bancarios                                    º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³TAMANHO NOSSONUM 033-040 = 8 CARACTERES                     º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function NossoNumSEE()
Local aArea := GetArea()
Local cNumDoc

If Empty(SE1->E1_NUMBCO)
	cNumDoc := StrZero((Val(Alltrim(SEE->EE_FAXATU))+1),8)
	RecLock("SEE",.f.)
	SEE->EE_FAXATU := cNumDoc
	MsUnlock()
Else
	cNumDoc := alltrim( substr(SE1->E1_NUMBCO,4,8) )
Endif

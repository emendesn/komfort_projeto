...
//ZK8_NOMCLI,ZK8_CPFCNP,ZK8_QTDPAR, ZK8_VLRPAR,ZK8_VLRENT,ZK8_PRIVEN,ZK8_COBRAR, ZK8_VLRDEV,ZK8_XUSER,ZK8_OBSWRF,ZK8_NEWTIT,ZK8_ALTVEN,ZK8_ANEXO
    //TITULOS PENDENTES
...
Static Function fCarrParam()
    ...
    @  34, 05 Say  oSay Prompt 'Titulo'	FONT oFont11 COLOR CLR_BLUE Size  30, 08 Of oPainel Pixel
    @  44, 05 MSGet oTitulo Var cTitulo FONT oFont11 COLOR CLR_BLUE Pixel SIZE 30, 05 VALID {|| processa( {|| fCarrTit(cFil, cTitulo, dEmissaoDe, dEmissaoAt) }, "Aguarde...", "Atualizando Dados...", .F.)} When .T. Of oPainel 
   ...
   // tButton():New(44,400,"&Conciliar",oPainel,{|| alert('Conciliar') },40,10,,,,.T.,,,,/*{|| }*/)
    tButton():New(44,050,"&Visualizar",oPainel,{|| fShow() },40,10,,,,.T.,,,,/*{|| }*/)
    tButton():New(44,105,"&Liberar",oPainel,{|| fInfCliente() },40,10,,,,.T.,,,,/*{|| }*/)
...
Static Function fCarrTit(cFilSel, cTit, dDtEmiDe, dEmiAte)
    ...
    cQuery := "SELECT ZK8_NOMCLI,ZK8_CODCLI,ZK8_CPFCNP,ZK8_QTDPAR, ZK8_VLRPAR,ZK8_VLRENT,ZK8_PRIVEN,ZK8_COBRAR, ZK8_VLRDEV,ZK8_XUSER,ZK8_OBSWRF,ZK8_NEWTIT,ZK8_ALTVEN,ZK8_ANEXO "
    cQuery += " FROM ZK8010"
    cQuery += " WHERE ZK8_WFRET <> 5 "
    cQuery += " AND ZK8_NEWTIT = '' "
    cQuery += " AND D_E_L_E_T_ <> '*'"
    if !Empty(alltrim(cTit))
        cQuery += " AND ZK8_CODCLI = '"+ cTit +"'"
        cQuery += " WHERE  ZK8_NEWTIT = ''"
        cQuery += " AND D_E_L_E_T_ <> '*'"
    endif
    ...
    */
    ...
    if len(aTitulos) == 0
        AAdd(aTitulos, {oSt1,oNo,"","","",0,"","","NãO",CTOD("//"),"","","",""," ",""})
    endif
...
Static Function fShow()
    ...
    if nLinha > 0
    	...
    	ZK8->(DbSeek(xFilial()+ aTitulos[nx][04]))
		   ...
		   if aTitulos[nx][09] <> 'NãO'
		        CpyS2TEx(alltrim(ZK8->ZK8_ANEXO), alltrim(cTemp + substr(ZK8->ZK8_ANEXO,rat("\",ZK8->ZK8_ANEXO)+1,len(ZK8->ZK8_ANEXO))))
...
Static Function fInfCliente()
	...
	Local nLinha := 0
    Private aCpos := {"ZK8_NEWTIT"}
	Private cCadastro := "Cobranca - Alteração de Cadastro"
    ...
    if nLinha > 0
	    ...
	    ZK8->(DbSetOrder(1)) //ZK8_FILIAL,ZK8_CODCLI, R_E_C_N_O_, D_E_L_E_T_
	     if ZK8->(DbSeek(xFilial()+ Alltrim(aTitulos[nx][04])))
	        AxAltera("ZK8",ZK8->(Recno()),4,,aCpos)
	    endif

...
User Function agend()
	...
	Private aPerg01 := {"1-TODOS","2-SIM","3-NÃO"}
	...
	Private aCores := {}
	Private cUserName := Alltrim(UsrRetName(__cUserId))
	Private cUsGer := SUPERGETMV("KH_ALLPED", .T., "000455|000478|000455")
	Private oComboBo11 
	
	If !(__cUserid $ cUserAge)	
		Aviso("Acesso não permitido - KH_USRAGE"  , "Usuário sem permissão de acesso")		
		Return		
	EndIf
	...
	//If (__cUserId = '000478') // Somente o Isaías terá acesso a rotina de consulta do log de estorno
...
Static Function vldPendF(_cMsfil,_cPedido)
	...
	if cPedPend == '2'
		msgAlert("Não é possivel agendar pedidos com Pendência Financeira !!","ATENÇÃO")
		lRet := .T.
	endif	
...
Static Function vldPendFC(cCliente, cLoja)
	...
	If (cAlias)->(!Eof()) .And. (!RetCodUsr()$cUserCh)//Usuários chave podem abrir chamados diferentes de 000008 e 000012                                        
		MsgAlert("O Cliente possui pendencia financeira não será permitido o agendamento. KH_SAC001. Entre em contato com o supervisor da área","ATENÇÃO")
		lRet := .F.
	EndIf   	    		    
	(cAlias)->(DbCloseArea())                       
...
Static Function CriaBotoes()
	...
	Local cMsg := "Deseja realmente estornar a liberação do pedido: "
    ...
    oEstorna := tButton():New(aTamBot[1],aTamBot[2],"&Estornar Pedido",oBotoes,{|| ; 
    																				iif(oGridIt:acols[oGridIt:nat][nPLocal] ='15',MsgAlert("Ação não permitida para o Armazém 15"),iif(msgyesno(cMsg+oGridPed:acols[oGridPed:nat][nPPedido],cTitulo),;
																					eval(bCodBloq),.F.)),;
...
Static Function CriaParam()
    ...
    @  23,  180 MSCOMBOBOX oOpcoes VAR nOpcoes ITEMS {"Codigo","Descrição"} SIZE 044, 010 OF oCampos COLORS 0, 16777215 PIXEL
	...
	If (__cUserId == '000478')
		//'Legenda
		@  44,  540 Say  oSay Prompt 'F10 - Consulta histórico de estorno'		FONT oFont11 COLOR CLR_RED Size  100, 08 Of oCampos Pixel
	EndIf

	//'Msg => Observações do Pedido e Entrega
	@  34,  540 Say  oSay Prompt 'Observações F11'		FONT oFont11 COLOR CLR_RED Size  50, 08 Of oCampos Pixel
	...
	@  36,  230 Say  oSay Prompt 'Desconto:'		FONT oFont11 COLOR CLR_BLUE Size  60, 08 Of oCampos Pixel
   	...
   	//'Bloqueio de credito
	@  36,  330 Say  oSay Prompt 'Armazém:'		FONT oFont11 COLOR CLR_BLUE Size  60, 08 Of oCampos Pixel
   	//TComboBox():New( 35,360, {|u|if(PCount()>0,nArmaze:= val(u),nArmaze)} ,aPerg05, 70, 05, oCampos, ,/*bMudaFiltro*/,,,,.T.,,,.F.,{||.T.},.T.,,)
   	@ 035, 360 MSCOMBOBOX oComboBo11 VAR cArmaze ITEMS {"01","03","04","12","Todos"} SIZE 070, 005 OF oCampos COLORS 0, 16777215 PIXEL
   	
	//'Pedido Faturado
	@  36,  440 Say  oSay Prompt 'Pedido Faturado:'		FONT oFont11 COLOR CLR_BLUE Size  60, 08 Of oCampos Pixel
	@  30,  495 RADIO oRadPedFat VAR cMV_PAR14 ITEMS "Sim","Não" SIZE 45, 25 OF oCampos COLOR 0, 16777215  ON CHANGE {|| processa( {|| fCarr() }, "Aguarde...", "Atualizando Dados...", .f.)} PIXEL

Return

//Função utilizada para definir o tamanho dos botoes
...
Static Function fHeadIt() 
	...
	aAdd(aHeaderIt,{"Endereço","C6_LOCALIZ",PesqPict("SC6","C6_LOCALIZ"),TamSX3("C6_LOCALIZ")[1],TamSX3("C6_LOCALIZ")[2],,"",TamSX3("C6_LOCALIZ")[3]	,"","","","",,'V',,,})
...
Static Function fCarr()
	...
	cQuery += " ORDER BY C5_NUM, C5_EMISSAO "+CRLF
...
Static Function fCarrIt(cPedido,cCliente,cLoja)
	...
	While (cAlias)->(!eof())
		...
		//Identifacção de produtos fora de linha
		iif((cAlias)->B1_01FORAL == 'F',cForaLinha := "SIM",cForaLinha := "NÃO")
...
Local j := 0
	...
	If !Dbseek(cPerg)
		...
		Aadd(aRegs,{cPerg,"02","Filial até  ?" 			,"","C","mv_ch2","C",04,0,0,"G","","MV_PAR02","","","","","","","","","","","","","","","","","","","","","","","","","SM0",""})
		Aadd(aRegs,{cPerg,"03","Data Abertura de ?"		,"","D","mv_ch3","D",08,0,0,"G","","MV_PAR03","","","","","","","","","","","","","","","","","","","","","","","","","",""})
		Aadd(aRegs,{cPerg,"04","Data Abertura até ?"	,"","D","mv_ch4","D",08,0,0,"G","","MV_PAR04","","","","","","","","","","","","","","","","","","","","","","","","","",""})
		...
		Aadd(aRegs,{cPerg,"07","Cliente até  ?" 		,"","C","mv_ch7","C",08,0,0,"G","","MV_PAR07","","","","","","","","","","","","","","","","","","","","","","","","","SA3",""})
		Aadd(aRegs,{cPerg,"08","Loja Cliente até  ?"	,"","C","mv_ch8","C",02,0,0,"G","","MV_PAR08","","","","","","","","","","","","","","","","","","","","","","","","","",""})
		Aadd(aRegs,{cPerg,"09","Pedido de  ?" 			,"","C","mv_ch9","C",06,0,0,"G","","MV_PAR09","","","","","","","","","","","","","","","","","","","","","","","","","SC5",""})
		Aadd(aRegs,{cPerg,"10","Pedido até  ?" 			,"","C","mv_ch10","C",06,0,0,"G","","MV_PAR10","","","","","","","","","","","","","","","","","","","","","","","","","SC5",""})
		Aadd(aRegs,{cPerg,"11","Data Entrega de ?"		,"","D","mv_ch11","D",08,0,0,"G","","MV_PAR11","","","","","","","","","","","","","","","","","","","","","","","","","",""})
		Aadd(aRegs,{cPerg,"12","Data Entrega até ?"		,"","D","mv_ch12","D",08,0,0,"G","","MV_PAR12","","","","","","","","","","","","","","","","","","","","","","","","","",""})
		Aadd(aRegs,{cPerg,"13","Agendado ?"				,"","N","mv_ch13","N",01,0,0,"C","","MV_PAR13","Todos","","","","","Sim","","","","","Não","","","","","","","","","","","","","","",""})
		aAdd(aRegs,{cPerg,"14","Pedido Faturado ?"		,"","N","mv_ch14","N",01,0,0,"C","","MV_PAR14","Sim","","","","","Não","","","","","","","","","","","","","","","","","","","",""})
	Endif
...
Static Function ITAGEND()
	...
	Aadd(aPerg,{PadR("ITAGEND",10),"04","Data Entrega até ?"	,"MV_CH04" ,"D",008,00,"G","MV_PAR04",""     ,""      ,""			,"","",""	,""   		})
	...
	Else
		MsgInfo("Não foram encontrados dados para os parametros informados.","ATENÇÃO")
	EnDIF	
...
Static Function TELMANU(aCols)                        
	...
	if nRadAgreg == 2
		MsgAlert("Não é possivel realizar agendamentos com a opção (Sem Agregados) selecionada.","ATENÇÃO")
		Return
	endif
	
	If nMostrua == 2
		MsgAlert("Não é possivel realizar agendamentos com o filtro Mostruario igual a NÂO selecionado.","ATENÇÃO")
		Return
	EndIf                                  
	...
	for nx := 1 To Len(aItens)
		if SC6->(DbSeek(xFilial("SC6") + aCols[nPPedido] + aItens[nx][nPItem] + aItens[nx][nPProduto])) //Pedido + Item + Produto
			if SC6->C6_ENTREG == stod('66660606') .AND. SC6->C6_LOCAL <> '12'
				msgAlert("Itens de desmontagem so podem sair do armazem 12, altere o armazem do item ("+ aItens[nx][nPItem] +").","Atenção")
				nCont++
				loop
			endif
		endif
	...
	//Alteração solicitada pelo chamado 9628
	DbselectArea("DAI")
	DAI->(dbsetorder(4))
	If DAI->(DbSeek(xFilial("DAI")+xPedCar))
		cMsgCarga := "O pedido: "+ xPedCar +" ja possui carga montada"+ENTER
		cMsgCarga += "Não é permitido sua alteracao. Entre em contato com a Logistica!"
    ...
    @ 046, 003 GROUP oGroup2 TO 115, 192 PROMPT " Informações do Cliente - " + alltrim(aCols[nPNome])  OF oDlg COLOR 0, 16777215 PIXEL
	...
	@ 246, 003 GROUP oGroup5 TO 270, 192 PROMPT " Restrição de Entrega " OF oDlg COLOR 0, 16777215 PIXEL
    ...
    If Empty(cRegCon)
			MsgAlert('Campo restrição de entrega vazio. Agendamento não efetuado.' ,'Atenção')
			Return
	EndIf
			...
			if SC6->(DbSeek(xFilial("SC6") + aCols[nPPedido] + aItens[nx][nPItem] + aItens[nx][nPProduto])) //Pedido + Item + Produto
				if empty(alltrim(SC6->C6_NOTA))
					if aItens[nx][nPQAtu] > 0 // Verifica se tem estoque
						...
						cEndereco := ""
						lRet := .F.
						//Obs: incluida validação devido o campo estar Negativo
	...
	if nCont > 0
		MsgAlert("Existem produtos nesse pedido com bloqueio de estoque, o agendamento ocorrerá somente nos itens com saldo","Atenção")
	endif
...
Static Function vldDate(dGetDtAgend,cCombo)
	...
	If  !__cUserid $ _cUser  //validação para apenas os usuarios do parametro KH_USBLQAG conseguirem agendar sem validar a data de bloqueio de agendamento.
	...
	if dGetDtAgend < dDataBase
		apMsgAlert("Não é possivel informar uma data menor que a data base.","WARNING")
		return(.F.)
	endif 
	...
	If !dtoc(dGetDtAgend) == '22/12/2222' .and. !dtoc(dGetDtAgend)  == '08/08/8888' //Validação para excluir estas duas datas da validação de domingo.Número do ticket: 13057
		if alltrim(DiaSemana(dGetDtAgend)) == 'Domingo'
			apMsgAlert("Não é possivel agendar entrega aos domingos.","WARNING")
			return(.F.)
		endif
	EndIf	
...
Static Function AGENDITEM(aCols, aColsItem)
	...
	if SC5->( dbSeek( xfilial() + cPedido ) ) 
		if SC5->C5_XLIBER == "B"
			msgAlert( "O Pedido de Venda: "+ cPedido +" está Bloqueado por conta de Desconto nos itens, verificar!", "Agendamento de Pedidos" )
			return
		endif
	endif
	...
	//Veriica se o cliente possiu títulos em aberto de pagamento ipedindo o agendamento de entrega dos produtos  - Marcio Nunes - 03/09/2019
	...
	if SC6->C6_ENTREG == stod('66660606') .AND. SC6->C6_LOCAL <> '12'
		msgAlert("Itens de desmontagem so podem sair do armazem 12, altere o armazem do item ("+ cItem +").","Atenção")
		Return
	endif

	// Validação// Não permitir agendar produtos Bipartidos no agendamento por item.
	dbSelectArea("SB1")
	SB1->(dbSetOrder(1))
	...
	if SB1->(dbSeek(xfilial("SB1")+cCodigo))
		...
		msgStop("Produto: "+ cCodigo +" Não foi localizado!!" ,"ATENÇÃO")	
	endif
	
//Não permitir alteracao para pedidos que já tenham carga montada chamado 9628
DbselectArea("DAI")
DAI->(dbsetorder(4))
...
Endif
	...
	//Selecionar os itens de termo retira que serão agendados na mesma data
	fViewTerRe(_cAtend, aTerRet)
		
	if len(aTerRet) > 0 .and. len(aTermAgend) == 0
		if !msgYesNo("Nenhum Termo foi selecionado, deseja prosseguir com o agendamento?","ATENÇÃO")
			Return
		endif
	endif
    ...
    @ 002, 003 GROUP oGroup1 TO 055, 192 PROMPT "Informações do pedido - " + cPedido OF oDlgItem COLOR 0, 16777215 PIXEL
    ...
    @ 024, 010 SAY oSayDesc PROMPT "Descrição :" SIZE 029, 007 OF oDlgItem COLORS 723931, 16777215 PIXEL
    ...
    @ 057, 003 GROUP oGroup2 TO 126, 192 PROMPT " Informações do Cliente - " + cNomeCli OF oDlgItem COLOR 0, 16777215 PIXEL
	...
	@ 128, 003 GROUP oGroup3 TO 174, 192 PROMPT " Observações do Pedido de venda " OF oDlgItem COLOR 0, 16777215 PIXEL
	@ 136, 006 GET oMsgPed VAR cMsgObsPed MEMO SIZE 182, 037 PIXEL OF oDlgItem
	
    @ 176, 003 GROUP oGroup4 TO 243, 192 PROMPT " Observações de Entrega " OF oDlgItem COLOR 0, 16777215 PIXEL
	@ 184, 006 GET oMsgEnt VAR cMsgObsEnt MEMO SIZE 182, 057 PIXEL OF oDlgItem
	
	//Restrição de entrega
	@ 246, 003 GROUP oGroup5 TO 270, 192 PROMPT " Restrição de Entrega " OF oDlgItem COLOR 0, 16777215 PIXEL
...
Static Function fViewTerRe(_cAtend, aTermos)
	...
	@ 024, 004 SAY oText PROMPT "Selecione os Termos Retira que serão agendados e procione o botão (Gravar Itens)." SIZE 150, 014 OF oDlg1 COLORS 255, 15658734 PIXEL
...
Static Function fMarcOne(nLin) 
	...
	if oMSNew1:aCols[nLin,nPStatus] == "3"
		MsgAlert("Este termo ja foi Finalizado.","ATENÇÃO !!")
	else
...
Static Function fTerRet(_cAtend, aTermos)
	...
	DEFINE MSDIALOG oDlg TITLE "Relação de Termos Retira" FROM 000, 000  TO 500, 500 COLORS 0, 16777215 PIXEL
		...
		if len(aTermos) > 0                                                       
			...
			msgAlert("Não existe termo retira para esse Pedido.","ATENÇÃO..")
			Return
        endif
...
Description //Tela de consulta da observações (observações do pedido) e (observações do Entrega)
@param xParam Parameter Description
@return xRet Return Description
@author  - Alexis Duarte
@since 28/09/2018
...
Static Function ViewObserv(aCols)
	...
	DEFINE MSDIALOG oDlgView TITLE "Observações do Pedido " + aCols[nPPedido] FROM 000, 000  TO 500, 390 COLORS 0, 16777215 PIXEL
	
		@ 003, 002 GROUP oGroup1 TO 108, 192 PROMPT " Observações do Pedido " OF oDlgView COLOR 0, 16777215 PIXEL
    	@ 011, 006 GET oMsgPed VAR cMsgObsPed MEMO SIZE 182, 95 PIXEL OF oDlgView	    
    	
	    @ 112, 003 GROUP oGroup2 TO 230, 192 PROMPT " Observações de Entrega " OF oDlgView COLOR 0, 16777215 PIXEL
...
Static Function fBloqAgend()
	...
	If !__cUserId $ cUserBlq
		return(apMsgAlert("Usuario sem permissão para utilizar esse recurso!!!","WARNING"))
	EndIf
...
Static Function fSalvar(dDataBlq, nOpc) //nOpc --> 1=Incluir, 2=Excluir
	...
	if nOpc == 1 //Enclusão
		...
		if dDataBlq < dDataBase
			return(apMsgAlert("Não é permitido informar data menor que data base!","WARNING"))
		endif
...
Description //Utiliza os pedidos filtrados para trazer as informações referente aos produtos
@param xParam Parameter Description
@return xRet Return Description
...
Static Function fExcelIt()
	...
	_cQuery := " SELECT C5_NUM, C5_EMISSAO, C6_PRODUTO, C6_ITEM, C6_DESCRI, C6_QTDVEN, C6_LOCAL, C6_VALOR, " +CRLF
	_cQuery += " C6_ENTREG, C6_01AGEND,C6_CLI,A1_NOME, C6_LOJA,C6_NUMTMK, " +CRLF
	_cQuery += " C6_MSFIL,C6_FILIAL,C6_NOTA, C6_QTDEMP, C6_LOCALIZ, C6_XUSRAGE,C6_XNUMSC, B1_01FORAL,C5_01SAC " +CRLF
	...
	_cQuery += " ORDER BY C5_NUM, C5_EMISSAO "+CRLF
	 ...
	 PLSQuery(_cQuery, cAliEm)
		...
		while (cAliEm)->(!eof())
			...
			_cQryRet := "SELECT ZK0_COD, ZK0_PROD, ZK0_DESCRI, ZK0_NUMSAC, ZK0_CLI, ZK0_LJCLI, ZK0_DTAGEN, ZK0_NUMSAC, ZK0_STATUS, ZK0_CARGA"+ ENTER
			_cQryRet += " FROM "+RetSqlName("ZK0")+ ENTER
			_cQryRet += " WHERE ZK0_CLI = '"+ (cAliEm)->C6_CLI +"'"+ ENTER
			_cQryRet += "	AND ZK0_LJCLI = '"+ (cAliEm)->C6_LOJA +"'"+ ENTER
			_cQryRet += " AND D_E_L_E_T_ = ' '"
		
			cAlias1 := getNextAlias()
		
			plsQuery(_cQryRet,cAlRet)
		
			aReTerm := {}
			_cAtend := (cAlRet)->ZK0_NUMSAC // Numero do Atendimento SAC
			...
			cDescfil := U_RETDESCFI((cAliEm)->C6_MSFIL)

			iif((cAliEm)->B1_01FORAL == 'F',cForaLinha := "SIM",cForaLinha := "NÃO")

			Aadd(aItPed,{	cDescfil,;				//1
							(cAliEm)->C6_CLI,; 		//2
							(cAliEm)->A1_NOME,;		//3
							(cAliEm)->C5_NUM,;		//4										
							(cAliEm)->C5_EMISSAO,;  //5										
							...
							cAgend,;				//18
							(cAliEm)->C6_NOTA,;  	//19
							(cAliEm)->C6_LOCALIZ,;	//20
							(cAliEm)->C6_XNUMSC,; 	//21
							(cAliEm)->C6_XUSRAGE}) 	//22
				(cAliEm)->(DbSkip())
		end 
				(cAliEm)->(dbCloseArea())
	
		
	 aCab := {{"Filial","C6_MSFIL","C"},; 			//1
		 	  {"Cod_cli","C6_CLI","C"},; 			//2
			  {"Nome","A1_NOME","C"},;  			//3
		 	  {"Pedido","C5_NUM","C"},; 			//4
			  {"Emissao","C5_EMISSAO","D"},; 		//5
	 		  {"Produto","C6_PRODUTO","C"},;		//6
	 		  {"Item","C6_ITEM","C"},;				//7
	 		  {"Descricao","C6_DESCRI","C"},;		//8
	 		  {"Qtd Ven","C6_QTDVEN","C"},;			//9
	 		  {"Local","C6_LOCAL","C"},;			//10
	 		  {"Valor","C6_VALOR","N"},;			//11
	 		  {"Entrega","C6_ENTREG","D"},;			//12
	 		  {"Saldo","C6_SALDO","N"},;			//13
	 		  {"Estoque","C6_SALDO","C"},;			//14																	
			  {"DescTipo","C6_DESCRI","C"},;		//15																
			  {"ForaLinha","B1_01_FORAL","C"},;		//16																	
			  {"Termo","C6_01AGEND","C"},;			//17
			  {"Agenda","C6_01AGEND","C"},;			//18 
			  {"Nota","C6_NOTA","C"},;  			//19
			  {"Ender","C6_LOCALIZ","C"},;			//20
			  {"Ped_comp","C6_XNUMSC","C"},; 		//21
			  {"Usr_Agend","C6_XUSRAGE","C"}}  		//22
...
Static Function atuDtAgend(acols)
	...
	Local lfConfirm := .F.
	Local oSayData
	Local cSayTitle := "Atualização da data de agendamento"
	Local oComboDt
	Local aItens := {'22/12/2222-Sem Previsão','03/03/3333-Cancelamento','04/04/4444-Bloqueado','05/05/5555-Procon-TJ','06/06/6666-Desmontagem','07/07/7777-Vistoria','08/08/8888-Contato sem Sucesso','09/09/9999-Não Agendado'}
...
Endif
	...
	if lfConfirm
		...
		If nMarca == 0
			msgAlert("Nenhum item foi selecionado!!","Atenção")
			return
		endif
...
Static Function faTuDtZK0(cPedido, dDataZk0 , lEstorno)
	...
	If (lEstorno)
		...
		Do While !lContinua
			...
			//Validacao para verificar se o motivo do estorno foi preenchido. Somente permitirá prosseguir se o motivo do estorno for informado.
			...
			*/		
			...
			If ( !(Empty(AllTrim(cTGet1))) .AND. (Len(AllTrim(cTGet1)) > 20))
				//Valida se o conteudo esta preenchido. Caso 'Esc' seja pressionada, valida se o conteudo da variavel é superior a 20. 
				//Se não for, força a digitação do usuário
	...
	If (lEstorno)
		...
		//Atualiza a tabela de log de alterações
...
//--------------------------------------------------------------
/*/{Protheus.doc} atuDtAgend
Description //Valida a data digitada na função (atuDtAgend)
@param xParam Parameter Description
@return xRet Return Description
@author  - Alexis Duarte
@since 04/10/2018
...
Description //Define a ação com base na coluna clicada no Pedido
@param xParam Parameter Description
@return xRet Return Description
@author  - Alexis Duarte
@since 07/12/2018 /*/
    ...
    Do Case
        ...
        iF ! empty(_array[nPEmail])
			if msgYesNo("Deseja Enviar email para: "+ alltrim(_array[nColuna]),"Contato Komfort House")
				...
				//Retorna a mensagem de acordo com a opção selecionada.
		...
		Else
		MsgAlert("Não existe nenhum registro selecionado","Atenção")
		EndIf
			
		Otherwise
			
			//Validação de pedido com pendencia financeira
			lPendF := vldPendF(_array[nPMsFil],_array[nPPedido])
			
			//Veriica se o cliente possiu títulos em aberto de pagamento ipedindo o agendamento de entrega dos produtos
...
Static Function getOpcMail()
	...
	@ 000, 002 GROUP oGroup TO 030, 146 PROMPT " Opções de notificação automatica " OF oDlgHtml COLOR 0, 16777215 PIXEL
...
Static Function getHtml(nOpc)
	...
	if nOpc == 1
        ...
        cbody += '		Seu estofado já esta disponível para entrega. Estamos tentando entrar em contato nos telefones cadastrados, porém sem sucesso.'
      	cbody += '	</p>'
    	cbody += '</h3>'

		cbody += '<h3>'
      	cbody += '	<p style="font-family: Arial; color: #495157;">'
        cbody += '		Por favor, entre em contato conosco e agende sua entrega nos telefones: <span style="font-family: Arial; color: black;">(11) 4280-4610 ou (11) 4343-3989</span>, de Segunda à Sexta- feira das 8hs às 18hs e aos sábados das 8hs às 16h20.'
      	cbody += '	</p>'
    	cbody += '</h3>'
    
    	cbody += '<h3>'
      	cbody += '	<p style="font-family: Arial; color: #495157;">'
        cbody += '		Aproveite e atualize seus dados cadastrais(endereço de entrega, e-mail e telefones) através de um e-mail Para o SAC:<span style="font-family: Arial; color: black;"> serviraocliente@komforthouse.com.br</span>'
        cbody += '	</p>'
		cbody += '</h3>'
		...
		cbody := '<h1 style="font-family: Arial; color: #495157;">'
		cbody += '	<br>Olá Cliente!'
		cbody += '</h1>'
		
		cbody += '<h3>'
		cbody += '	<p style="font-family: Arial; color: #495157;">'
		cbody += '		Seu estofado já esta disponível em nosso Estoque.'
		cbody += '	</p>'
		cbody += '</h3>'
		...
		cbody += '      De Segunda à sexta 8h às 18h, Sábado das 8h às 16h20.'
		cbody += '  </p>'
		cbody += '</h3>'
...
Description //Função responsavel pelo envio de email para os clientes
@param xParam Parameter Description
@return xRet Return Description
@author  - Alexis Duarte
@since 21/01/2019 /*/
...
Description //Define a ação com base na coluna clicada no Item do pedido de venda
@param xParam Parameter Description
@return xRet Return Description
@author  - Alexis Duarte
@since 31/01/2018 /*/
          	...
          	SC6->(dbgoto(aPed_[nx][1])) //Posicione no registro da SC6
     		  IF !EOF()
     		  	RecLock("SC6",.F.)
      		  		SC6->C6_XVENDA := '1'
      		  	SC6->(msUnLock())
      		  	MsgAlert("Produto Liberado com Sucesso!","Atenção!")
      		  EndIF 		 
      	Next nx
Else
MsgAlert("Nenhum produto encontrado!","Atenção!")
EndIf
SC6->(dbCloseArea())
Return 
...
Description //Tela de consulta ao histórico de estornos
@param xParam Parameter Description
@return xRet Return Description
@author  - Luis Artuso
@since 19/12/2019 /*/
...
Static Function ConsEst()
	...
	Local cAlias 	:= "ZLO"
	Local cTitulo	:= "Consulta ao histórico de estornos"
	Local cVldExc	:= "U_fVldAge(1)" //Validação na Exclusão
	Local cVldOk	:= "U_fVldAge(2)" //Validação na Inclusão
	dbSelectArea(cAlias)
...
Static Function fVldSai(cTGet1,oDlg)
	...
	If !(Empty(cTMP))
		...
		If (Len(cTMP) < 20)
			...
			Aviso("Campo obrigatório" , "Informe quantidade superior a 20 caracteres para prosseguir com o estorno.")
	...
	Else
		...
		Aviso("Campo obrigatório" , "É obrigatório informar o motivo do estorno do pedido.")
...
Description //Validacao para impedir alterações na tabela de log de estorno
@param xParam Parameter Description
@return xRet Return Description
@author  - Luis Artuso
@since 06/01/2020 /*/
...
User Function fVldAge(nOp)
	...
	Do Case
		...
		Case (nOp == 1)
			...
			MsgAlert("Nao é permitido excluir o registro")
		...
		Case (nOp == 2)
			...
			Do Case
				...
				Case (INCLUI)
					...
					MsgAlert("Somente será permitida a inclusão através do estorno do pedido")

				Case(ALTERA)

					MsgAlert("Registro indisponível para manutenção!")
...
Static Function ConfEst()
	...
	If (lFLinha) 
		...
		Else
			Aviso("ESTORNO INVÁLIDO" , "Não é possível prosseguir com o estorno, pois constam produtos fora de linha no pedido. Entre em contato com o gestor da área")
		EndIf
	Else	
		staticCall(TMK150EXC,fAltPed,cMsfilEst,cPedidoEst,.T.)
		faTuDtZK0(cPedidoEst,stod('99910909') , .T.)

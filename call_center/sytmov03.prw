...
±±ºPrograma  ³ SYTMOV03 º Autor ³ LUIZ EDUARDO F.C.  º Data ³  31/08/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ GERA AS NOTAS DE TRANSFERENCIA PARA A VENDA DE MOSTRUARIO  º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  º  GERAPED º Autor º LUIZ EDUARDO F.C.  º Data ³  17/10/2016 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºObjetivo  ³ GERA PEDIDO DE TRANSFERENCIA NA FILIAL DE ORIGEM DO PRODUTOº±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±                              `
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
...
If !SA1->(DbSeek(xFilial("SA1") + ALLTRIM(aPrdMost[1,nFILSAI]))) 
	Aviso("SYTMOB03 - Atenção!!!","Cliente correspondente a filial destino "+ALLTRIM(aPrdMost[1,nFILSAI]) +;
...
//cNumSC5 := U_fGerREG(1) //#CMG20180612.n - Substituido pois a numeração não iniciou como 000001 no GoLive	//#RVC20180613.o
...
If lMsErroAuto
	...
	Return(.F.)
Else
	MsgInfo("Pedido de transferência incluido com sucesso. Número : " + cNumSC5 + " - Filial : " + cFilAnt)
	...
	cMensagem := "Aos cuidados do departamento de logística da Komfort House,"+chr(13)+chr(10)+chr(13)+chr(10)
	cMensagem += "Por favor providenciar o faturamento do pedido n.° "+ cNumSC5 + " proveniente do retorno simbólico da Loja " + _cFilial +"." +chr(13)+chr(10)+chr(13)+chr(10)
	cMensagem += chr(13)+chr(10)+chr(13)+chr(10)
	cMensagem += "Faz-se urgente e primordial o faturamento deste e sua devida entrada no CD Principal (Armazém de Mostruários) após transmissão do DANFe."+chr(13)+chr(10)+chr(13)+chr(10)
	cMensagem += "O pedido n.º "+ _cPedOrig +" do cliente "+ _cCliOrig +" depende desta transferência para ser faturado."+chr(13)+chr(10)+chr(13)+chr(10)
	cMensagem += chr(13)+chr(10)+chr(13)+chr(10)
	...
	Processa( {|| fEnvMail(_cEmail,"[LOJA "+_cFilial+"] Retorno de Mostruário - PV N.°: " + cNumSC5,"\Transf_Mostruario\"+cFilAnt+"_" + ALLTRIM(cNumSC5) + ".TXT",cMensagem)},"Enviando e-mail para a área de logística...")

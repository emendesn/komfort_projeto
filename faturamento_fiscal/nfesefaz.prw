	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				//--------- específico KomfortHouse
				_aArea := getArea()
				cQuery := "select distinct E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,"
				cQuery +=	"E1_PIS,E1_COFINS,E1_CSLL,E1_INSS,E1_VLRREAL,E1_IRRF,E1_ISS"
				cQuery +=	" from "+retSqlName("SE1")+" SE1 (noLock) "
				cQuery +=	" join "+retSqlName("SD2")+" SD2 (noLock) on D2_FILIAL='"+SF2->F2_FILIAL+"'"
				cQuery +=		" and E1_PREFIXO='PED' and D2_PEDIDO=E1_NUM and SD2.D_E_L_E_T_=' '"
				...
				TcSetField(cAliasSE1,"E1_VENCORI" ,"D", 8, 0 )
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//³ K035 - IMPRIME O NUMERO DA CARGA NO CAMPO OBSERVACOES DA DANFE - LUIZ EDUARDO F.C. - 09.01.2018 ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						//cMensCli := cMensCli + CRLF + "N.Carga : " + SF2->F2_CARGA	//#RVC20180522.o
						//#RVC20180522.bn
						If Empty(aK035)
							Aadd(aK035,{Alltrim(SF2->F2_CARGA)})
							cMensCli := cMensCli + CRLF + "N.Carga: " + Alltrim(SF2->F2_CARGA)
						Else
							For _nKY := 1 to Len(aK035)
								If AScan(aK035,{|x| AllTrim(x[1]) == AllTrim(SF2->F2_CARGA)}) > 0
									Loop
								EndIf
								cMensCli := cMensCli + CRLF + "N.Carga: " + alltrim(aK035[nKI][1])
							Next
						EndIf
						//#RVC20180522.en
...
If !Empty(aNota)
	...
	//--------- específico KomfortHouse
	aEspVol := reCalc( aProd )				// Especie, volumes e pesos -  Cristiam Rossi em 10/07/2017
	//--------- específico KomfortHouse
	...
	//--------- específico KomfortHouse
	MsgKomfort( @cMensFis, @cMensCli, aPedido, aNota, aProd, aDest, cNota+cSerie+cClieFor+cLoja )		// Mensagens da NF - Cristiam Rossi
	//--------- específico KomfortHouse
...
//-----------------------------------------------------------
// Especifico KomfortHouse - Mensagens na NF - Cristiam Rossi
//-----------------------------------------------------------
Static Function MsgKomfort( cMensFis, cMensCli, aPedido, aNota, aProd, aDest, cKeySF )
Local aArea    := getArea()
...
Local cPosipi
Local cCFOP
Local lVenda   := aNota[4] == "1" .and. aNota[5] == "N"		// Saída e Normal
Local lSaiDev  := aNota[4] == "1" .and. aNota[5] == "D"		// #CMG20181002.n
Local lEntDev  := aNota[4] != "1" .and. aNota[5] == "D"		// Entrada e Devolução
Local cUFDest  := aDest[9]
Local cMsgNew  := ""
Local cMsgTmp  := ""
Local cPV      := ""
Local lTroca   := .F.
	...
	for nI := 1 to len( aProd )
		cMsgTmp := ""
		cPosipi := alltrim(aProd[nI,05])
		cCFOP   := alltrim(aProd[nI,07])
		cPV     := aProd[nI,38]

		if left(cPosipi,4) == "9401" .and. cCFOP $ "5102/1202" .and. cUFDest == "SP"
			cMsgTmp := Formula("001")
		endif
...
//#CMG20181002.bn
//Adiciona número do pedido nas notas de devolução para o fornecedor - chamado 5904
	if lSaiDev
		cMsgTmp := "Pedido N.: "+ cPV +"| "
		cMsgNew += iif(empty(cMsgNew),"","| ") + cMsgTmp
	endif
		...
		if SC5->( dbSeek( xFilial("SC5") + cPV ) )
			...
			if ! empty( SC5->C5_01SAC )
				...
				if SUC->( dbSeek( xFilial("SUC") + SC5->C5_01SAC ) )
					...
					if lTroca
//						cMsgTmp := "Troca/retorno referente ao pedido: " + SUC->UC_01PED + iif( !empty(cMsgTmp), "| Motivo: "+cMsgTmp, "")
	...
	if lEntDev
		SF1->( dbSetOrder(1) )
		if SF1->( dbSeek( xFilial("SF1") + cKeySF ) )
			SD1->( dbSetOrder(1) )
			SD1->( dbSeek( xFilial("SD1") + cKeySF ) )
			lSUC := .F.
			while ! SD1->(EOF()) .and. SD1->(D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA) == xFilial("SF1") + cKeySF
				
				if ! empty(SD1->D1_01SAC)
					SUC->( dbSetOrder(1) )
					if SUC->( dbSeek( xFilial("SUC") + SD1->D1_01SAC ) )
						lSUC := .T.
						...
						while ! SUD->( EOF() ) .and. xFilial("SUD") + SUD->UD_CODIGO == xFilial("SUC") + SUC->UC_CODIGO
//							if SUD->UD_DATA == SC5->C5_EMISSAO .and. ! empty( SUD->UD_01TIPO )
							if ! empty( SUD->UD_01TIPO )
								if Z01->( dbSeek( xFilial("Z01") + SUD->UD_01TIPO ) ) .and. Z01->Z01_TPPEDI=='N' .and. Z01->Z01_TIPO=='3'	// dev
									if ! alltrim( SUD->UD_OBS ) $ cMsgTmp
										cMsgTmp += iif(empty(cMsgTmp),"","| ") + alltrim( SUD->UD_OBS ) + "| "
									endif
								endif
			...
			if lSUC
				cMsgNew += iif(empty(cMsgNew),"",". ") + "MOTIVO: " + cMsgTmp + "| "

				if ! empty( SUC->UC_01PED )
					cMsgNew += iif(empty(cMsgNew),"","| ") + "Pedido N.: "+ SUC->UC_01PED +"| "
				endif
				SC5->( dbSetOrder(1) )
				if ! empty( SUC->UC_01PED ) .and. SC5->( dbSeek( xFilial("SC5") + alltrim(SUC->UC_01PED) ) )
					cMsgTmp += "Representante: "+ alltrim( Posicione("SA3",1,xFilial("SA3")+SC5->C5_VEND1,"A3_NOME") ) + "| "
					cMsgNew += iif(empty(cMsgNew),"","| ") + cMsgTmp
				endif
			endif
	...
	cMensCli += iif(empty(cMensCli),"","| ") + cMsgNew
...
//--------------------------------------------------------------------
// Especifico KomfortHouse - Especie, Volumes e pesos - Cristiam Rossi
//--------------------------------------------------------------------
Static Function reCalc( aProd )
Local aArea := getArea()
Local aRet  := {}
Local nI
	...
	for nI := 1 to len( aProd )
		...
		nPos := aScan( aRet, {|it| it[1]==cEsp})
		if nPos == 0
			aadd( aRet, {cEsp,0,0,0} )
			nPos := len( aRet )
		endif

		nQtE := iif( SB5->B5_QE1==0, 1, SB5->B5_QE1 )

		aRet[nPos, 2] += int( nQuant / nQtE )
		aRet[nPos, 3] += nPLiq * nQuant
		aRet[nPos, 4] += nPBrt * nQuant
	next

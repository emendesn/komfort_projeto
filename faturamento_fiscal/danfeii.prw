...
//----- KonfortHouse - Cristiam Rossi em 25/05/2017
if lRet
	U_PrintEtiq()	// chama impressão das etiquetas
endif
//------------------------------------------------- 
...
If nFaturas > 0
	For nX := 1 To 3
		aAux := {}
		For nY := 1 To 1	//Min(10, nFaturas)	// Min(9, nFaturas)
			Do Case
				Case nX == 1
					SE1->( dbSetOrder(1) )
					If nFaturas > 1
//						AAdd(aAux, AllTrim(oFatura:_Dup[nY]:_nDup:TEXT))
						SE1->( dbSeek( xFilial("SE1") + AllTrim(oFatura:_Dup[nY]:_nDup:TEXT) ) )
					Else
//						AAdd(aAux, AllTrim(oFatura:_Dup:_nDup:TEXT))
						SE1->( dbSeek( xFilial("SE1") + AllTrim(oFatura:_Dup:_nDup:TEXT) ) )
					EndIf
					AAdd(aAux, SE1->(E1_PREFIXO + E1_NUM) )
				Case nX == 2
//					If nFaturas > 1
//						AAdd(aAux, AllTrim(ConvDate(oFatura:_Dup[nY]:_dVenc:TEXT)))
//					Else
...
//						AAdd(aAux, AllTrim(TransForm(Val(oFatura:_Dup:_vDup:TEXT), "@E 9,999,999,999,999.99")))
//					EndIf
					AAdd(aAux, AllTrim(TransForm(Val(oTotal:_ICMSTOT:_vNF:TEXT), "@E 9,999,999,999,999.99")))
			EndCase
		Next nY
		If nY <= 10	// 9
			For nY := 1 To 10	// 9
...
cQuery := " TOP 1 D1_NFORI AS NFORI FROM "+RetSqlName("SD1")+" SD1 WHERE SD1.D1_DOC='"+SF1->F1_DOC+"' AND  SD1.D1_SERIE='"+SF1->F1_SERIE+"' AND  SD1.D1_FORNECE='"+SF1->F1_FORNECE+"' AND  SD1.D1_LOJA='"+SF1->F1_LOJA+"' AND SD1.D_E_L_E_T_=''""
...
cQuery := " TOP 1 D2_PEDIDO AS PEDORI FROM "+RetSqlName("SD2")+" SD2 WHERE SD2.D2_DOC='"+xNforig+"' AND  SD2.D2_SERIE='"+SF1->F1_SERIE+"' AND  SD2.D2_CLIENTE='"+SF1->F1_FORNECE+"' AND  SD2.D2_LOJA='"+SF1->F1_LOJA+"' AND SD2.D_E_L_E_T_=''""
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(045,003,cLogoD,080,090)
	Else
		oDanfe:SayBitmap(045,003,cLogo,080,090)
	EndIF
Endif
...
For nY := 1 To nLenItens
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If lMv_Logod
			oDanfe:SayBitmap(000,003,cLogoD,095,096)
		Else
			oDanfe:SayBitmap(000,003,cLogo,095,096)
		EndIf
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(043,003,cLogoD,095,096)
	Else
		oDanfe:SayBitmap(043,003,cLogo,095,096)
	EndIF
Endif

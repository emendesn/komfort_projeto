...
SELECT C5_MSFIL FILIAL, FILIAL DESC_FIL, C5_ORCRES ORCAMENTO, C6_NUM PEDIDO_VENDA,SUBSTRING(C6_PRODUTO,1,3) FORNE, 
C6_PRODUTO PRODUTO, B1_CODANT COD_NETGERA, B1_DESC DESCRICAO,C6_QTDVEN QUANTIDADE, C6_NOTA + ' ' + C6_SERIE NOTAFISCAL,
CASE WHEN B1_XENCOME = '2' THEN 'ENCOMENDA' ELSE 'GIRO' END ENCOMENDA,
SUBSTRING(C5_EMISSAO,7,2)+'/'+SUBSTRING(C5_EMISSAO,5,2)+'/'+SUBSTRING(C5_EMISSAO,1,4) EMISSAO, 
ISNULL(C1_NUM,'') SOLICITACAO, ISNULL(C1_PEDIDO,'') COMPRA, ISNULL(C1_ITEMPED,'') ITEM,
SUBSTRING(C6_ENTREG,7,2)+'/'+SUBSTRING(C6_ENTREG,5,2)+'/'+SUBSTRING(C6_ENTREG,1,4) ENTREGA,
ISNULL(A2_NREDUZ,'SEM FORNECEDOR') FORNECEDOR, 
ISNULL(B2_QATU,0) ESTOQUE
FROM SC6010 SC6 (NOLOCK)
INNER JOIN SC5010 SC5 (NOLOCK) ON C5_FILIAL = C6_FILIAL AND C5_NUM = C6_NUM
INNER JOIN SB1010 SB1 (NOLOCK) ON B1_COD = C6_PRODUTO AND SB1.%notDel%
INNER JOIN SM0010 SM0 (NOLOCK) ON C5_MSFIL = FILFULL
LEFT OUTER JOIN %table:SB2K% SB2 (NOLOCK) ON SB2.%notDel% AND B2_FILIAL = '0101' AND B2_COD = C6_PRODUTO AND B2_LOCAL = C6_LOCAL
LEFT OUTER JOIN SC1010 SC1 (NOLOCK) ON SC1.%notDel% AND C1_FILIAL = '0101' AND C1_PEDRES = C6_NUM AND C1_PRODUTO = C6_PRODUTO
LEFT OUTER JOIN SA2010 SA2 (NOLOCK) ON SA2.%notDel% AND B1_PROC = A2_COD AND B1_LOJPROC = A2_LOJA     
WHERE SC5.%notDel%
AND SC6.%notDel%
AND C5_NUM <> ''
AND C5_CLIENT <> '000001'

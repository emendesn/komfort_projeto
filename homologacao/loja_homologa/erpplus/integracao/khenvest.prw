...
User function KHENVEST()
	...
	//cA1		:= "D39243EF-5943-4378-A3EB-AA3D58865678"
	//cA2		:= "39132A70-1622-43D5-AABD-B504DC924D26"
		...
		if oPedido:oWsListarNovosResult:nCodigo == 1
			...
			For nAux := 1 to len(aPedidos)
				...
				For nX := 1 to len(aItens)
					...
					nPos := aScan(aSaldoSite,{ |x| Alltrim(x[1]) == AllTrim(aItens[nX]:CCODIGOINTERNO) })

					if nPos > 0
						aSaldoSite[nPos][2] += aItens[nX]:NITEMQTDE
					Else
						//aAdd(aSaldoSite,{CODIGOKIT,QUANTIDADE})
						aAdd(aSaldoSite,{aItens[nX]:CCODIGOINTERNO,aItens[nX]:NITEMQTDE})
					Endif
	...
	While !( TZ07->( Eof() ) )
		...
		Endif
			...
			If  len(alltrim(TZ07->Z07_PRODUT)) < 12
				If oEstoque:Salvar(nLoja,cCodProd,,nQtdEst,nQtdMin,nTipoAlt,cA1,cA2) 

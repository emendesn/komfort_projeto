...
user function KHCADPVE()
	...
	While .Not. ( TPZ12->( Eof())) 
		...
		aAdd(aPedidos,{"C5_XORIGEN",TPZ12->Z12_ORIGEM, Nil})
		...
		While .Not. ( TPZ12->( Eof())) .and. cPeAtu == TPZ12->Z12_PEDECO
			...
			SKUSEEK(TPZ12->Z12_PRODUT,TPZ12->Z12_QUANT,nTotalPrd,TPZ12->Z12_DESC,@aItens,lPadrao)
		...
		if !lPadrao
			GERASC6(aItens,cPedGerado,xFilial("SC6"))
		Else
...
DbSetOrder(1)
	...
	Replace C5_XORIGEN With aCabec[aScan(aCabec,{|x| x[1] == "C5_XORIGEN" })][2]
	//Replace C5_TRANSP  With aCabec(aScan(aCabec,{|x| x[1] == "C5_LOJACLI" }))
...
*/
Static Function GERASC6(aLinha,cNumSc5,cFilAtu,cPendente)
...
Static Function SKUSEEK(cCodKit,nQuant,nTotal,nDesc,aItens,lPadrao)
	...
	//cQuery += CRLF + "	OR ZKC_SKU= '"+cCodKit+"' "

...
User Function KHENVICLI()
    ...
    cEndPoint := ALLTRIM(GETNEWPAR("KH_ENDRECL","http://trustvox.com.br"))
    ...
    Aadd(aHeader, "Authorization: Bearer SLEYyzsnaxvs4t-7khhG" ) 
    //informo qual é a pasta dentro do endereço do webservice
    oRest:setPath("/api/stores/109491/orders")
 ...
 DbSelectArea('Z20')
   ...
   for nx := 1 to len(aClie)
	  ...
	  oCliente['items'] := {}
		      ...
		      oItems['id']   := Alltrim(aProd[nz][2])
		      oItems['name'] := Alltrim(aProd[nz][1])
		      oItems['url'] := "http://komforthouse.com.br"
...
Local  aPprod := {}
    ...
    cQue02 := CRLF + "SELECT Z21_DESCPR, Z21_CODPRO    FROM Z21010 "
    cQue02 += CRLF + "WHERE Z21_PEDIDO = '"+cPedi+"' "
    cQue02 += CRLF + "AND D_E_L_E_T_ = '' "
    	  
    PlSquery(cQue02, cTMPZ21 )
    
    While (cTMPZ21)->(!EOF())
    aAdd(aPprod,{(cTMPZ21)->Z21_DESCPR,;
    			(cTMPZ21)->Z21_CODPRO})
    	(cTMPZ21)->(DbSkip())		
    End
    	(cTMPZ21)->(DbCloseArea())
